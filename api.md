...
<font color=red>暂不支持语音合成相关功能</font>
...
info|string|是|type为1，2，3，5时，是图片url，当type为4时，如："欢迎到来"
...
apk_url|string|是|apk的url，用于下载apk
...
|capture_image|base64|是|人脸现场抓拍照片（base64)|
|score|float|是|比对得分|
|card_image|base64|是|身份证小图|
|card_name|string|否|身份证照片姓名|
|card_id|string|是|身份证id|
|camera_id|string|是|流id|
|timestamp|int64|是|毫秒级时间戳|
...
recognize_type|识别类型|int64|1表示1:n，2表示1:1；
...
auth_url|二次鉴权接口|string|进行二次鉴权接口
recognize_type|识别类型|int64|1表示1:n，2表示1:1；人证一体机应选择2
capture_max_interval|最大尝试人脸抓拍时长(ms)|int64|值大于等于1
